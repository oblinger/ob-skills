...
---
:>> [[{NAME}]] → [[{NAME} Docs]] → [[{NAME} Dev]] → [[{NAME} Architecture]]
...
The `Arch` row points to the **most-specific** architecture destination — the per-module arch doc if one exists, otherwise the subsystem arch doc, otherwise the top-level `{NAME} Architecture.md` (single-subsystem case). Every API doc has exactly one such row. See `[[CAB Architecture]]` § Bidirectional cross-linking.
...
- **Not for layer-wide contracts** — those belong in `[[CAB Interface]]` (the human-authored layer contract that groups modules). Don't pile architecture-level discussion here either; that goes in `[[CAB Architecture]]` and the per-anchor `Arch` row points to it.
