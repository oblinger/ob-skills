...
Body-only — no YAML frontmatter. The first content line is the `# CAB Status` H1; the second is the `description::` dataview inline field above; everything else is plain markdown. (Same body-only discipline as [[CAB Rules]].)
...
Available to any anchor with a `{NAME} Design/` folder per [[CAB Design]]. v1 facet list (`prd`, `ux`, `architecture`, `testing`, `roadmap`) matches the canonical `/design` phase set; per-trait facet-list customization is Phase 2.
...
- [[CAB Testing]] — peer Design facet whose `status:: accepted` is consumed by `/design`'s gate.
...
**Why:** body-only matches the broader vault discipline ([[CAB Rules]], [[CAB Backlog]] are also body-only). Frontmatter is invisible in normal Obsidian read view and easy to drift.
