...
def audit(anchor_path: str, verbose: bool = False) -> list[Finding]:
    ...
    for rel_path in sorted(sources.keys()):
        if rel_path not in files_md_entries:
            # Check if it's there with a slightly different path
            basename = os.path.basename(rel_path)
            if not any(basename in e for e in files_md_entries):
                n += 1
                findings.append(Finding(n, rel_path, "missing-entry",
                                        f"Source file not in Files.md"))

    # ── Compare Files.md → source ──
    for entry in sorted(files_md_entries):
        if entry not in sources:
            # Check if basename exists somewhere
            basename = os.path.basename(entry)
            if not any(os.path.basename(s) == basename for s in sources):
                n += 1
                findings.append(Finding(n, entry, "stale-entry",
                                        f"Files.md entry — file not in source"))

    # ── Compare source → module docs ──
    for rel_path in sorted(sources.keys()):
        basename = os.path.basename(rel_path)
        stem = os.path.splitext(basename)[0]
        # Expected doc name: {RID} {PascalCase}.md
        # Try several naming conventions
        found_doc = False
        for doc_name in module_docs:
            # Strip RID prefix and compare
            short = doc_name.replace(f"{rid} ", "")
            if short.lower() == stem.lower() or short.lower().replace(" ", "_") == stem.lower():
                found_doc = True
                # Check freshness
                source_mtime = git_mtime(sources[rel_path]) or file_mtime(sources[rel_path])
                doc_mtime = git_mtime(module_docs[doc_name]) or file_mtime(module_docs[doc_name])
                if source_mtime and doc_mtime and source_mtime > doc_mtime:
                    ...
                    if diff > 3600:
                        ...
                        findings.append(Finding(n, rel_path, "stale",
                                                f"Doc '{doc_name}' is {staleness} behind source",
                                                staleness))
                break
    ...
    print(f"Files.md: {len(files_md_entries)} entries, {missing_entries} missing, {stale_entries} stale", file=sys.stderr)
    print(f"Module docs: {len(module_docs)} exist, {missing_docs} missing, {stale_docs} stale, {unlinked} unlinked", file=sys.stderr)
