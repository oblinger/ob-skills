...
**Bidirectional cross-linking**: every module doc carries an `Arch` row in its top-of-doc dispatch table (see [[CAB API Doc]]) pointing at the most-specific architecture destination. `/architect` maintains both directions.
...
|---|---|
| Public API surface (modules, classes, functions, signatures) | `{NAME} API.md` (sub-doc inside `{NAME} Architecture/`); follows [[CAB API Doc]] rules |
...
- [[CAB API Doc]] — companion spec for the public-API sub-document.
- [[CAB Rules]] — rule-set format spec. Diagrams in architecture docs are audited against rule sets cited by the anchor's decisions.
...
- [[CAB Decisions]] — anchor-level applied choices that cite rules from rule sets.
- [[CAB Design Dispatch]] — Architecture sits alongside PRD / Decisions / Interface in `{NAME} Design/`.
...
- **NOT a tutorial, not a worked example, not API-doc rules.** Walk-throughs live at [[CAE Architecture]]; API-doc structure lives at [[CAB API Doc]]; per-anchor decisions live in `{NAME} Decisions/`. Link to those — don't inline their content here.
