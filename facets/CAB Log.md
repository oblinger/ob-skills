...
- [[CAB Roadmap]] — sibling facet (forward plan, not past narrative)
- [[CAB Anchor Page]] — the anchor's home; should link to `[[{NAME} Log]]`
...
**Check pattern:** grep `{NAME} Log.md` for `^# BRIEF` or `^# Brief`. If present and its body contains general log-format prescriptions (filename pattern, body convention, "don't duplicate spec content"), flag for migration to point at [[CAB Log]] instead.
...
- **What does NOT belong here:** per-anchor log content (lives in `{NAME} Log/`), Brief-style restatements of these rules on per-anchor dispatch pages (forbidden by R-log-07), or convention/spec material that belongs in sibling facets like [[CAB Backlog]] or [[CAB Roadmap]].
