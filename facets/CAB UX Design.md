# CAB UX Design
description:: facet spec for `{NAME} UX Design.md` — the *human* user-facing surface (CLI commands, screens, organization, naming, output shapes, error voice). Sibling peer to [[CAB API Design]] (programmatic surface).

UX Design specifies the **human user surface** of the anchor — how a person discovers, invokes, reads, and recovers from errors. It is sibling to [[CAB API Design]] (the *programmatic* user surface). The cut between them is **who the consumer is**, not where the surface lives: UX = human reading or invoking; API Design = code calling.

> [!info] Scope guard
> "UX" here is the broad sense — *user-facing surface* — not narrowly "visual interface." For a CLI scheduler the UX is the command set, output shapes, and error voice. For a GUI app it is screens + flows. For a skill repository (no pixels) it is the slash-command surface and organization. If the anchor has no human consumer, this facet is N/A; mark it `none` in [[CAB Status]] and omit the file.

The UX Design doc is the **current spec** — what the surface looks like today, not what alternatives were considered. Rationale lives in [[CAB Decisions]] or in the document's own *D-UX<n>* design-decision rows (see [[#RULESET R-ux|R-ux]]). Exhaustive flag/option reference for CLIs lives in the sibling [[CAB CLI]] doc; UX Design captures the *intent* (what commands exist, what they mean, what output they produce), CLI doc is the *reference* (every flag, every exit code).
...
Peer of [[CAB Architecture]] and [[CAB API Design]] under [[CAB Design Dispatch|Design]].
...
| `## Design decisions` | `D-UX<n>` rows: each load-bearing UX choice with rationale (why this and not the obvious alternative). Bridge to [[CAB Decisions]] for decisions that cite a rule set. |
...
- [[CAB API Design]] — sibling facet covering the programmatic surface.
- [[CAB CLI]] — exhaustive flag/exit-code reference (downstream of UX Design's *intent*).
- [[CAB Decisions]] — load-bearing decisions citing rules; bridge from D-UX rows.
- [[CAB Architecture]] — internal organization; UX entry-points typically map to architecture components.
...
- The programmatic surface — that's [[CAB API Design]].
- The exhaustive flag/option reference — that's [[CAB CLI]] (for CLI anchors).
- The internal organization — that's [[CAB Architecture]].
...
- **Preserve the facet-cut boundaries** — UX Design owns *human-facing intent*; [[CAB API Design]] owns *programmatic surface*, [[CAB CLI]] owns *exhaustive flag reference*, [[CAB Architecture]] owns *internal organization*. When editing, never absorb content that belongs in a sibling facet; rule R-ux-08 is the load-bearing guard.
- **Rule set is co-located, not separate** — the `# RULESET R-ux` H1 at the bottom is part of this file per F133. When adding or revising rules, update the rule block in place; do not split it into a sibling file or duplicate it in [[CAB Rules]].
- **Cross-references are load-bearing** — links to [[CAB API Design]], [[CAB CLI]], [[CAB Decisions]], [[CAB Architecture]], [[progressive-disclosure]], and [[CAE UX Design]] anchor the facet's place in the wider blueprint. Renaming or moving any of those files requires updating the wiki-links here in the same commit.
