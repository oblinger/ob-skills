...
- **Linking conventions.** Refer to sibling facets by their CAB filename in wiki-links (`[[CAB Inbox]]`, `[[CAB Backlog]]`); refer to per-anchor instances with the `{NAME}` placeholder (`{NAME} Messages.md`), never a concrete anchor's name.
- **Cross-references to keep in sync when this spec changes.** [[CAB Base]] dispatch tables, [[CAB All Files]] tree, and any anchor template that scaffolds a `{NAME} Messages.md` file.
