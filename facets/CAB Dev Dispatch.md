...
| Module docs | [[CAB API Doc]] — one row per documented module, grouped by source folder |
...
- **Interface** ([[CAB Interface]]) — required top-level human-authored layer contract. Lives in `{NAME} User/{NAME} Interface.md`.
...
- **Inclusion test for new content:** does it apply to *every* `{NAME} Dev.md` in *every* code anchor? If yes, edit here. If it's anchor-local, edit the anchor's Dev dispatch. If it's a synthesis-zone rule, edit [[CAB User Dispatch]] instead.
...
- **Cross-references to keep current:** [[CAB Files]], [[CAB API Doc]], [[CAB Interface]], [[CAB User Dispatch]], [[CAE Dev]] (working example). If any of these slugs rename or move, update the wiki-links in the body — the dispatch contract refers to them by basename.
- **Cite, don't inline:** markdown rendering rules live in [[R-markdown]]; dispatch-table formatting rules live in [[CAB Anchor Page]]. Reference those rather than duplicating their content here.
