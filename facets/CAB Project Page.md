...
- **Cross-references to keep aligned**: [[code-publish]] (the workflow that builds + deploys), [[CAB Documentation Site]] (sibling facet for full doc sites), and the anchor's dispatch-table `External` row format shown in § Dispatch Table Entry. If any of those names change, update here.
