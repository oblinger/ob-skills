...
API Design specifies the **programmatic user surface** of the anchor — what shape another piece of code (or another agent invoking this as a sub-skill) sees when integrating. It is sibling to [[CAB UX Design]] (the *human* user surface). The cut between them is **who the consumer is**: API = code calling; UX = human reading or invoking.

> [!info] Scope guard
> "API" here means the *intent* of the programmatic surface — how it's shaped, what it commits to. Distinct from [[CAB API Doc]] (per-module reference; *what exists* in the source tree, often generated) and from [[CAB Architecture]] (internal organization; *how* the system is built). API Design is the *what* of the public contract.

The API Design doc is the **current spec** — the contract surface today. Rationale lives in [[CAB Decisions]] or in the document's own *D-API<n>* design-decision rows (see [[#RULESET R-api|R-api]]). If the anchor has no programmatic consumer (no library form, no sub-skill invocation surface, no integration story), this facet is N/A; mark it `none` in [[CAB Status]] and omit the file.
...
Peer of [[CAB Architecture]] and [[CAB UX Design]] under [[CAB Design Dispatch|Design]].
...
| `## Design decisions` | `D-API<n>` rows: each load-bearing API choice with rationale. Bridge to [[CAB Decisions]] for decisions citing a rule set. |
...
| Facet | Owns | Boundary |
|---|---|---|
| **[[CAB UX Design]]** | Human user-facing surface (CLI commands, screens, organization, output shapes for the eye). | Different *consumer*. |
| **[[CAB API Doc]]** | Per-module reference documentation (what exists in the source tree, often generated). | Different *altitude* — API Design is intent; API Doc is reference. |
| **[[CAB Architecture]]** | Internal organization (modules, dependency direction, layering). | Different *audience* — Architecture is for the system's builders; API Design is for consumers. |
| **[[CAB Interface]]** | Internal layer/component contracts (between subsystems within the anchor). | Different *visibility scope* — Interface is internal; API Design is the public surface. |
| **[[CAB CLI]]** | Exhaustive flag/exit-code reference for CLI binaries. | Different *form* — CLI doc is reference; API Design covers structured (`--json`) output contract when the CLI doubles as a programmatic surface. |

## See also

- [[CAB UX Design]] — sibling facet covering the human surface.
- [[CAB API Doc]] — reference documentation for the implemented modules.
- [[CAB Architecture]] — internal organization that backs the API surface.
- [[CAB Decisions]] — bridge for D-API rows that cite rule sets.
- [[CAB Status]] — `{NAME} Status.md` carries the API-Design facet state.
...
- The per-module reference of *what exists* — that's [[CAB API Doc]].
- The internal organization of the system — that's [[CAB Architecture]].
- The human-facing surface — that's [[CAB UX Design]].
...
- **The cut against neighbors is load-bearing — don't blur it.** API Design = *intent* of the programmatic surface; [[CAB API Doc]] = *what exists* in source (reference); [[CAB Architecture]] = *internal organization*; [[CAB UX Design]] = *human* surface. When tempted to add per-module inventories, dependency narratives, or screen flows here, redirect to the sibling facet — citing the redirect is fine, inlining the content is leakage.
...
- **When the facet is N/A for an anchor** (no library form, no sub-skill invocation surface, no integration story), the anchor marks it `none` in [[CAB Status]] and omits the file entirely; this facet spec does NOT change shape based on that — anchors opt out, the spec stays whole.
