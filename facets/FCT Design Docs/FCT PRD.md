...
- **Rule numbering is monotonic-forever** — `R-prd-NN` IDs are never recycled. New rules append at the next integer; deprecated rules are marked but their numbers stay reserved. Renumbering silently re-points every existing `R-prd-NN` cross-reference (including any rule-side `implements D<N>` back-links and docs citing a rule by id).
