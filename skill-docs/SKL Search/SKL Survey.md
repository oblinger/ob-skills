...
| -[[SKL Survey]]- | → [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[OBSK]] → [[SKL/SKL]] → [[SKL Search]] → [SKL Survey](hook://p/SKL%20Survey)<br>: skim → click into the rule that applies |
| --- | --- |
...
**Survey** builds a multi-dimensional comparison table across many entities + interpretive notes. Two meta-survey forms: **lightweight** (survey-of-surveys to derive dimensions) and **full** `/survey meta <noun>` (three composed surveys plus cell-certainty notation — see [[SKL Meta Survey]]). For comparison — not single-entity profiling ([[SKL Profile]]) or single-instance lookup ([[SKL Find]]).
