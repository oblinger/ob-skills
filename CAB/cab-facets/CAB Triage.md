...
- **`~/ob/kmr/Q.md`** — vault-level Agent Status dashboard. Each anchor's per-anchor section in Q.md is **identical content** to that anchor's Triage body (per F028 Q3). The H1-equivalent line links the entire `{NAME} Triage` phrase via `[[{NAME} Triage]]` (no pipe alias) — the link target matches what the user clicks, so jumping to the Triage page is unsurprising. On the Triage file's own H1, the bare `[[{NAME}]]` is the link (jumping to the anchor page) followed by the plain word "Triage" — symmetric: clicking what's underlined goes where you'd expect.
