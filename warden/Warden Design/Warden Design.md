...
The rule system spans more than the `/rule` skill — its prescriptive format lives in [[FCT Ruleset]], adoption in [[FCT Decisions]], the catalog in [[Rulesets]], and the runtime in [[Audit Architecture]]. [[Warden Architecture]] is the unified map that ties those together; the feature specs for the build live in [[Warden Features]].
