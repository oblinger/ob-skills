...
RULES = {
    ...
    "script-assisted": [
        ...
        "where:: `**/*.md`",
        "```",
        "# the oracle only IDENTIFIES; the rule writes the directive the agent sees",
        "q = file.section('## Open Questions')",
        "answer = ask_oracle(f'Which of these are answered elsewhere?'",
        "                    f' Reply \"none\" if none.\\n\\n{q}')",
        "if 'none' not in answer.lower():",
        "    tell(f'These look answered elsewhere — close or move to Resolved:\\n{answer}')",
        "```",
    ],
    "edit": [
