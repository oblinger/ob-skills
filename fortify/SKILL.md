...
2. **Batch open questions, never trickle them.**
   If user input is genuinely needed, gather all questions and surface them in one batch via the open-questions discipline. Never stop / ask / resume / stop / ask / resume.
...
3. **Fortify in cheap-to-expensive order:**
   ...
   6. (If warranted) Propose simplification or application-shape change → escalate to user via open-questions discipline.
