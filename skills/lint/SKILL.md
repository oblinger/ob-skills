...
- **Fix**: Add the class to the appropriate module doc's CLASSES table and create a per-class table with its fields and methods. Follow [[FCT API Doc]] format.
...
- **Fix**: Add the enum to the CLASSES table (with `Enum —` prefix in description) and create a two-column enum table listing its variants. See [[FCT API Doc]] for the enum table format.
...
- **Fix**: Create module docs following [[FCT API Doc]] format. Remember the **Linking Rule**: add to Dev dispatch table and Files FIRST, then write the doc.
