---
name: lint
description: Lint an anchor — static analysis against CAB type rules — Checks structural conformance, missing files, dispatch tables, module doc coverage. Use when user says: "lint this", "check the structure", "are there violations".
tools: Read, Write, Edit, Bash, Glob, Grep
user_invocable: true
---

# Lint — Static analysis of an anchor against CAB rules
...
5. **Exception** by adding a row to `.anchor.d/lint/exceptions.md` if the warning is noise
...
### What lint never does (per F059)

Lint is **detect-only**. Even when the finding implies a clear repair, lint's agent workflow MUST NOT take the repair action — that's `/rewire`'s job. Three categorical rules:

- **No dispatch-table edits.** If a finding is "missing row" or "missing dispatch table," lint flags it for `/rewire` — does **not** add the row or table. Auto-fixing a `dispatch-duplication` finding by adding a third canonical-form table is the DMUX bug (F059 root cause) — lint refuses.
- **No file creation.** Missing module docs / missing facet files are flagged for `/rewire` — lint does **not** create the file.
- **No moves.** Misplaced files (basename in wrong location) are flagged for `/rewire` — lint does **not** move them.

The `cab-lint.py` script is already read-only; this rule binds the **agent workflow** on top of the script. The split is structural: lint reports, rewire repairs. Bundling detection and repair in one skill is what produced the duplication bug; F059 separates them.

### For `class-undocumented` warnings
A class exists in source but has no entry in a module doc.
- **Fix**: Add the class to the appropriate module doc's CLASSES table and create a per-class table with its fields and methods. Follow [[FCT Module Doc]] format.
...
- **Fix**: Add the enum to the CLASSES table (with `Enum —` prefix in description) and create a two-column enum table listing its variants. See [[FCT Module Doc]] for the enum table format.
...
- **Fix**: Create module docs following [[FCT Module Doc]] format. Remember the **Linking Rule**: add to Dev dispatch table and Files FIRST, then write the doc.
...
Located at `.anchor.d/lint/exceptions.md` in the anchor. Sorted by module path, then target.
