...
description: >
  ...
  surface the inbox. On second press (user re-invoking after a
  no-action exit), lowers the threshold for autonomous action and
  skips the futile /groom; if still stuck, names the specific
  blocker. Slash-only — "crank" is NOT a DMUX prefix-trigger (too
  common in speech); `'` is the dedicated keystroke.
...
### 4. Mint loop (sequential portion)

```
minted_count = 0
minted_ids = []
while True:
    # First-press: pick the highest-priority safely-mintable Ready item.
...
- **Second press**:
  - If the lowered-threshold loop in step 4 produced a mint, fall through to the success branch above.
