...
- [ ] **If `{NAME} Interface.md` is absent:** auto-create a scaffold (H1 + canonical dispatch placeholder + TODO sections per [[CAB Interface]] § Document Structure) AND file a `## Now` backlog row in `{NAME} Backlog.md`: `**F<n> — Author top-level Interface for {NAME}** [Designing] — Rewire scaffolded {NAME} Interface.md on {YYYY-MM-DD}. Needs user collaboration to author the layer contract — see [[CAB Interface]]. → [[{NAME} Interface]].` The agent does NOT attempt to fill in the contract content — that's the user-collaboration step per [[workflow]] § Interface-validation gate.
