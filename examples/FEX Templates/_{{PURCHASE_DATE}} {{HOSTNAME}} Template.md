...
{{what changed and why — may span paragraphs}}
### ...
...
- **Two placeholder forms:** `{{UPPER_SNAKE}}` is a **variable** — reused or structural, named and defined above; `{{lower-case description}}` is a **one-off**, described in place, no definition needed.
- **Single- vs multi-line:** an *inline* placeholder holds one line (`{{event title}}`); an *own-line* placeholder holds a block (`{{what changed …}}`, `{{free-form notes …}}`).
