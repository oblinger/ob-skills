...
---
# describe-book — default rules

Default rules for `describe` on noun-type **book**. Ships with the skill; user overrides in `[[SRC rules/describe-book|SRC rules/]]`.
...
(Empty — user adds in `SRC rules/describe-book.md` to override these defaults.)
