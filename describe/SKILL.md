...
2. `describe/rules/describe.md` — default verb rules (ships with skill).
3. `describe/rules/describe-<noun>.md` — default pair rules with entity knowledge baked in (ships with skill).
4. `SRC rules/describe.md` — user's verb-level overrides (if present).
5. `SRC rules/<noun>.md` — user's noun-level overrides, cross-verb (if present).
6. `SRC rules/describe-<noun>.md` — user's pair-level overrides (most specific, wins all).
...
2. **Identify the noun-type** (person / corp / product / …). Load matching pair-rules file (`describe/rules/describe-<noun>.md`) for entity dimensions, sources, output shape.
...
- Privacy-sensitive subject (private individual) → guard-rail question first (see `rules/describe-person.md`).
