...
Vault-wide — every anchor's files are subject to this set, no explicit `include::` required in `{NAME} Decisions.md`. Listed in the catalog for completeness.
...
- F141 (future R-anchor umbrella) — would collect R-naming + R-folder + R-anchor-page + R-files when those rule sets exist.
