...
The umbrella that **`/audit anchor <path|slug>`** resolves ([[F161 — Rule-driven audit engine — resolve, run, judge|F161]]). Auditing an anchor *includes* auditing its documents, so this umbrella `include::`s [[R-doc]] (markdown / file-association / ruleset / brief / discussion / log / messages) on top of the anchor-structural sets:
