...
description:: Everything checked when auditing a whole anchor — the entry page + naming + planning facets, plus the doc-level rule sets (via R-doc) for every document the anchor contains.

The umbrella that **`/audit anchor <path|slug>`** resolves ([[F161 — Rule-driven audit engine — resolve, run, judge|F161]]). Auditing an anchor *includes* auditing its documents, so this umbrella `include::`s [[R-doc]] (markdown / file-association / rule-set / brief / discussion / log / messages) on top of the anchor-structural sets:
