...
- **Q1 — Group membership: static at submit time or dynamic via tags?** — context: do tasks declare their group when submitted (static), or do groups select tasks by tag-match (dynamic)?
...
- **Q2 — Pause semantics for groups: pause new dispatch only, or include in-flight cancellation?** — parallel to F1 Q2 for the global pause; do groups inherit the same semantics or have their own?
  - (A) Same as F1 — pause new dispatch, in-flight runs to completion. Operator opts into hard-stop separately if needed.
  - (B) Group-pause is harder — pause + cancel in-flight tasks belonging to the group.
  - **Recommendation:** Lean (A). Match F1 for consistency; "harder pause" is a separate feature if it ever shows up.

- **Q3 — Visibility: should `status()` decompose by group, or expose only aggregate?** — operators may want per-group queue depth.
...
- **Q4 — UI rendering of task groups in `cae list`** — how does the CLI display grouped tasks?
