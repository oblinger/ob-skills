...
- **The `crank` Command** — `crank`, `'`, or `/crank`: outer-loop autonomous-progress driver. Picks Ready items and invokes `/mint` per item in a loop until natural stopping. If anything got minted, exits silently (user re-presses to continue). If nothing got minted, runs `/groom` + `/triage` as the no-action fallback. On second press (right after a no-action exit), lowers the threshold and skips the futile `/groom`; if still stuck, names the blocker. Full: `~/.claude/skills/crank/SKILL.md`.
