...
- **This file is the trait spec for Paper Anchor**, sibling to other `traits/*.md` files. It defines the on-disk shape, the version-table contract, and the Paper Flow review/merge semantics — when changed, downstream skills and audits that target Paper anchors must be re-checked.
...
- **Linking conventions**: refer to this trait as `[[Paper Anchor]]`; sibling traits live under `traits/`; the umbrella catalog is [[TRT]]. When citing this spec from a worked instance, link the trait name — never copy the spec body.
