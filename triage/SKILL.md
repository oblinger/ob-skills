...
3. **Rebracket in BOTH places**: the backlog row AND the rendered triage line. Fix the backlog row via the workflow skill's `backlog-edit.py` (`~/.claude/skills/workflow/scripts/backlog-edit.py {NAME} same <row-id> <NewStatus>`) — never edit the backlog file directly. The script preserves title and body and auto-refreshes Q.md. Then render the triage line with the new bracket.
