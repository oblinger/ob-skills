...
- **`/finalize`** — Finalize is end-of-feature ceremony: verify, commit, update docs and stat, archive the feature record. It applies when the current activity *is* a feature wrap-up. Land is the more general "finish whatever is in flight." If land's named activity is "complete the current feature," land delegates to `/finalize` for the ceremony.
